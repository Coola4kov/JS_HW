...
const map = {
    ...
    render(snakePointsArray, foodPoint) {
        ...
        // Получаем элемент для отображения значения счёта
        const counter = document.getElementById('counter');
        // Отображаем текущий счёт
        counter.innerHTML = snakePointsArray.length;
...
const snake = {
     ...
     * @param {int} totalCols Количество строк
     * @param {int} totalRows Количество столбцов
     */
    init(startBody, direction, totalCols, totalRows) {
        this.body = startBody;
    ...
    getNextStepHeadPoint() {
        ...
        switch (this.direction) {
            case 'up':
                if (firstPoint.y - 1 === -1) {
                    return {x: firstPoint.x, y: this.totalRows - 1}
                }
                return {x: firstPoint.x, y: firstPoint.y - 1};
            case 'right':
                if (firstPoint.x + 1 === this.totalCols) {
                    return {x: 0, y: firstPoint.y};
                }
                return {x: firstPoint.x + 1, y: firstPoint.y};
            case 'down':
                if (firstPoint.y + 1 === this.totalRows) {
                    return {x: firstPoint.x, y: 0};
                }
                return {x: firstPoint.x, y: firstPoint.y + 1};
            case 'left':
                if (firstPoint.x - 1 === -1) {
                    return {x: this.totalCols - 1, y: firstPoint.y};
                }
                return {x: firstPoint.x - 1, y: firstPoint.y};
        }
    },
